  ...
  def get(self):
    ...
    if url!="" :
      ...
      if result.status_code == 200:
        ...
        if(self.request.get("url")):
          cookie_val = ('host' + '=' + parseString[0]+"://"+parseString[1] + ";").encode()
          self.response.headers.add_header('Set-Cookie',cookie_val)
          cookie_val = ('absolute_path' + '=' + re.sub("/[^/]*$","",parseString[2]) + ";").encode()
          self.response.headers.add_header('Set-Cookie',cookie_val)
          cookie_val = ('img_proxy' + '=' + self.request.get("img_proxy") + ";").encode()
          self.response.headers.add_header('Set-Cookie',cookie_val)
        self.response.out.write(result.content)
    else:
      self.response.out.write("")

app = webapp.WSGIApplication([('/main', MainPage),(r'/[^?]*',PageProxy)], debug=True)
